...
class EPGWindow(xbmcgui.WindowXMLDialog):
    ...
    def setButtons(self, starttime, curchannel, row):
        ...
        offair = ADDON_SETTINGS.getSetting("Channel_" + str(channel) + "_offair")
        if offair == "":
            offair = 0
        if int(offair) == 1:
            # channel is off air
        ...
        elif int(chtype) == 8 or int(chtype) == 9:
    ...
    def onAction(self, act):
        ...
        if action == ACTION_PREVIOUS_MENU:
            self.closeEPG()
            ### TV TIME ###
            self.MyOverlayWindow.channelThread = ChannelListThread()
            self.MyOverlayWindow.channelThread.myOverlay = self.MyOverlayWindow
            self.MyOverlayWindow.channelThread.start()
            ###############
    ...
    def determinePlaylistPosAtTime(self, starttime, channel):
        ...
        chname = ADDON_SETTINGS.getSetting("Channel_" + str(channel) + "_3")
        chtype = ADDON_SETTINGS.getSetting("Channel_" + str(channel) + "_type")        
        if int(chtype) == 8 or int(chtype) == 9:
